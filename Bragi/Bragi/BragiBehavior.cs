...
namespace BRAGI.Bragi
{
    public class BragiBehavior : WebSocketBehavior
    {
        ...
        {
            Valhalla.Valhalla.Instance.LastMessageReceived = new DateTime();
            Console.WriteLine("Received Websocket Message: " + e.Data);
            WebSocketResponse resp = null;
            try
            {
                WebSocketMessage mes = JsonConvert.DeserializeObject<WebSocketMessage>(e.Data);
                ...
                else
                {
                    try
                    {
                        ...
                        if (!BragiCommands.Commands.ContainsKey(mes.Command))
                        {
                    ...
                    {
                        if (resp != null) Send(JsonConvert.SerializeObject(resp));
                    }
                }
            }
            catch(Exception er)
            {
                if (er is JsonSerializationException || er is JsonException || er is JsonReaderException)
                {
